...
export async function initStateStore(stateStore: IStateStore): Promise<void> {
    ...
    // }
    // const pairs = `${EnvVars.BASE_SYMBOL}${EnvVars.QUOTE_SYMBOL}`;
...
export async function buyConditionally(
    kraken: IKraken,
    ...
    try {
        ...
        if (EnvVars.NUMBER_OF_BUYS <= 0 || state.counter < EnvVars.NUMBER_OF_BUYS) {
            state = await buyAction(kraken, coinGecko, assetMapper, state);
...
async function buyAction(
    ...
    trades.forEach((trade) => {
        for (let i = 0 ; i < state.trades.length ; i++) {
            if (state.trades[i].pair === trade.pair) {
                state.trades[i].volume += Number(trade.volume);
            }
        }
    });
    state.counter++;
